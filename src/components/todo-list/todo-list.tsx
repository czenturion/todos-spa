      ...
      {tasks.length > 0 ? tasks.filter(task => filterFn(filter, task)).map((task, index) => (
        <li key={task.id}
          ...
          <div className="btn-toolbar ml-2">
            <button
              className="btn btn-manage btn-outline-success"
            ...
            <button
              className="btn btn-manage btn-outline-info ml-1"
              onClick={() => {
                setEditedTask(task.task)
                setEditMode({id: task.id, mode: true})
              }}
            >
              <span className="fa fa-pencil"></span>
            </button>
            <button
              className="btn btn-manage btn-outline-danger ml-1"
